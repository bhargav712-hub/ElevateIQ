...
import { FiBriefcase, FiAward, FiCpu, FiCalendar, FiCreditCard, FiTarget, FiClock, FiUsers, FiTrendingUp, FiBookmark, FiGlobe } from 'react-icons/fi';
...
export default function Career() {
  ...
  const tabItems = [
    ...
    { key: 'internships', label: 'Internships', icon: FiAward, count: internships.length },
    { key: 'projects', label: 'Project Training', icon: FiCpu, count: projectTraining.length },
  ];
            ...
            {tabItems.map(tab => {
              ...
              return (
                <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                  ...
                  style={{ borderRadius: 50, display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                  <TabIcon /> {tab.label} ({tab.count})
                </button>
              );
            })}
          </div>
              ...
              {internships.map((item, i) => (
                <motion.div key={i} className="card" {...fadeUp} transition={{ delay: i * 0.08 }}>
                  <div className="card-body">
                    ...
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
                      <span className="badge badge-primary" style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><FiCalendar /> {item.duration}</span>
                      <span className="badge badge-success" style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><FiCreditCard /> {item.stipend}</span>
                      <span className="badge badge-warning" style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><FiTarget /> {item.spots} spots</span>
                    </div>
              ...
              {projectTraining.map((item, i) => (
                <motion.div key={i} className="card" {...fadeUp} transition={{ delay: i * 0.08 }}>
                  <div className="card-body">
                    <h3 style={{ fontWeight: 700, marginBottom: 8 }}>{item.title}</h3>
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
                      <span className="badge badge-primary" style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><FiCpu /> {item.tech}</span>
                      <span className="badge badge-success" style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><FiClock /> {item.duration}</span>
                      <span className="badge badge-warning" style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><FiUsers /> {item.mentor}</span>
                    </div>
          ...
          <div className="grid-2" style={{ gap: 40, alignItems: 'flex-start' }}>
            <motion.div {...fadeUp}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                {[
                  { icon: FiTrendingUp, title: 'Career Growth', desc: 'Fast-track your career with hands-on experience' },
                  { icon: FiBookmark, title: 'Mentorship', desc: 'Learn from industry experts with 10+ years experience' },
                  { icon: FiGlobe, title: 'Global Exposure', desc: 'Work on projects with teams worldwide' },
                  { icon: FiAward, title: 'Certification', desc: 'Get certified and boost your resume' },
                ].map((item, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', gap: 12, padding: 16, borderRadius: 12, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }}>
                      <span style={{ display: 'inline-flex', fontSize: '1.4rem', color: 'var(--primary-light)' }}><Icon /></span>
                      <div>
